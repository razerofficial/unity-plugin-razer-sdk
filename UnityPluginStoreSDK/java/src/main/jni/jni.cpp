...
jint JNI_OnLoad(JavaVM* vm, void* reserved)
{
	...
	jclass clazz = env->FindClass("com/razerzone/store/sdk/engine/unity/InputView");
	if (clazz)
	{
...
#endif
			__android_log_print(ANDROID_LOG_INFO, LOG_TAG, "lib-ndk-unity-store-sdk: VERSION=%s", PLUGIN_VERSION);
		}
		else
		{
	...
	{
		__android_log_print(ANDROID_LOG_ERROR, LOG_TAG, "Failed to find InputView class");
		return JNI_ERR;
	}

#if VERBOSE_LOGGING
	__android_log_print(ANDROID_LOG_INFO, LOG_TAG, "JNI_OnLoad complete!");
#endif
